...
const FormularioContacto = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `${import.meta.env.VITE_REACT_APP_API_URL}/api/formulario/contenidos`
      )
      .then((res) => {
        const clasificaciones = [
          ...new Set(res.data.map((i) => i.Clasificacion)),
        ];
        setContenidos(clasificaciones);
      })
      .catch((err) =>
        console.error("Error al cargar contenidos:", err?.response?.data || err)
      );
  }, []);
    ...
    if (form.Clasificacion) {
      ...
      axios
        .get(
          ...
          }/api/formulario/programas/${encodeURIComponent(form.Clasificacion)}`
        )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_REACT_APP_API_URL}/api/formulario/enviar`,
        data
      );
  ...
  return (
    <section className="form-container mt-4" id="contacto">
      <div className="text-center text-primary fw-bold fs-3 mb-4">
        SOLICITA INFORMES
      </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              ...
              >
                ...
                {programas.map((p) => (
                  <option key={p.id_evento} value={p.id_evento}>
                    {p.Descripcion}
                  </option>
                ))}
              </select>
